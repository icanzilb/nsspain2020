...
var subscriptions = [UUID: Timelane.Subscription]()
...
struct AppState: Equatable {
    ...
    {
      willSet {
        let diff = newValue.elements
          .difference(from: todos.elements)
        for change in diff {
            switch change {
            case .insert(_, let element, _):
                let newSubscription = Timelane.Subscription(name: element.id.uuidString)
                newSubscription.begin()
                subscriptions[element.id] = newSubscription

            case .remove(_, let element, _):
                subscriptions.removeValue(forKey: element.id)?.end(state: .completed)
            }
        }
      }
    }
